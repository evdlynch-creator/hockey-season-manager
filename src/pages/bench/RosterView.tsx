...
import { Badge, Avatar, ScrollArea, Button } from '@blinkdotnew/ui'
...
interface RosterViewProps {
  ...
  onToggleUnit?: (playerIds: string[], forceOn?: boolean) => void
}
...
export function RosterView({
  ...
  const playersByUnit: Record<string, Player[]> = {}
  units.forEach(u => playersByUnit[u] = [])

  players.forEach(p => {
    const lineup = playerLineups.get(p.id)
    const unit = lineup?.unit ?? 'Unassigned'
    if (playersByUnit[unit]) {
      playersByUnit[unit].push(p)
    } else {
      playersByUnit['Unassigned'].push(p)
    }
  })
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <div className="flex items-center justify-between px-2">
        <h3 className="text-xs font-black uppercase tracking-widest text-zinc-500">Live Line Management</h3>
      ...
      <ScrollArea className="flex-1 -mx-4 px-4">
        <div className="space-y-6 pb-8">
          {units.map(unit => {
            const unitPlayers = playersByUnit[unit]
            if (unitPlayers.length === 0 && unit !== 'Unassigned') return null
            if (unitPlayers.length === 0 && unit === 'Unassigned') return null

            return (
              <div key={unit} className="space-y-2">
                <div className="flex items-center justify-between px-2">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-primary/60 italic">
                    {unit}
                  </h4>
                  {onToggleUnit && unit !== 'Unassigned' && (
                    <Button 
                      variant="ghost" 
                      size="sm" 
                      className="h-6 px-2 text-[8px] uppercase font-bold rounded-full hover:bg-primary/10 hover:text-primary"
                      onClick={() => onToggleUnit(unitPlayers.map(p => p.id))}
                    >
                      Toggle Unit
                    </Button>
                  )}
